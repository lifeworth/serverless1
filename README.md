...
```bash
serverless1$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
serverless1$ sam local start-api
serverless1$ curl http://localhost:3000/
```
...
```bash
serverless1$ sam logs -n HelloWorldFunction --stack-name serverless1 --tail
```
...
```bash
serverless1$ cd HelloWorldFunction
...
```bash
sam delete --stack-name serverless1
```
